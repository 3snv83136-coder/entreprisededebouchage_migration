...
import { getAllGeoEvier13Slugs, getGeoEvier13Page } from '@/lib/data/geo-evier-13';
...
export async function generateStaticParams() {
  return getAllGeoEvier13Slugs().map((slug) => ({ slug }));
}
...
export async function generateMetadata({
  ...
  const page = getGeoEvier13Page(slug);
  if (!page) return {};

  const title = page.metaTitle;
  const description = page.metaDescription;
  const canonical = `${BASE_URL}/debouchage-evier-13/${slug}/`;
  const place =
...
export default async function DebouchageEvierVarSlugPage({
  ...
  const breadcrumbs = [
    ...
    { name: page.breadcrumbLabel, href: `/debouchage-evier-13/${slug}/` },
  ];
